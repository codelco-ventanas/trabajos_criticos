...
### Paso 6 · Dejar el mapa en cero

Esta versión del esquema ya no crea datos de ejemplo: la aplicación parte vacía.

Si en una instalación anterior quedaron los tres trabajos de demostración, ejecuta
`sql/limpiar-datos-ejemplo.sql` en el SQL Editor para eliminarlos.
...
Al guardar un trabajo, el propio formulario ofrece un botón **Respaldo PDF** con el
comprobante de la declaración. No requiere clave.
...
El historial **no tiene límite de tiempo**: los registros quedan para siempre. Nunca se
borra un trabajo automáticamente, sólo su archivo de permiso.

Un registro pesa alrededor de 1 KB. Con 50 trabajos diarios son unos 18 MB al año, sobre
los 500 MB gratuitos: daría para más de veinte años. En la práctica, el historial no es
un problema de espacio.

Lo único acotado es la **consulta**: cada búsqueda devuelve como máximo 500 registros, y
la pantalla avisa cuando llega a ese tope para que acotes las fechas. Es un límite de
lectura, no de almacenamiento: los registros siguen todos ahí.
...
Se generan dos documentos, ambos dentro del navegador (no consumen nada del plan de
Supabase y funcionan igual en el celular). Los dos llevan el encabezado institucional:
antetítulo *Codelco Ventanas*, el título del documento, el aviso de derechos y el logo
en la esquina superior derecha.

### Respaldo de declaración

Aparece **dentro del formulario, después de guardar** el trabajo. Es el comprobante de
que la persona declaró la actividad: incluye todos los datos ingresados, la ubicación
marcada en el plano, el identificador del registro y una línea para la firma del
supervisor responsable.

### Historial

Desde la ventana de Historial, con los botones **PDF** y **CSV**, que exportan los
resultados de la búsqueda. El PDF deja escritos en el encabezado los filtros aplicados
y el período consultado, para que el documento impreso diga siempre a qué universo de
datos corresponde. Las horas de inicio y término van en columnas separadas, junto a una
columna de fecha. El CSV abre directamente en Excel con los acentos correctos.

---

## 9. Iconos y logo

Los tres iconos de tipo de trabajo están en `assets/iconos/` como siluetas blancas con
transparencia. La aplicación los usa como máscara CSS, lo que permite teñirlos del color
que corresponda en cada lugar: blancos dentro del marcador del mapa, y del color del
tipo en los filtros y las tarjetas del panel.

Para reemplazar alguno, deja el archivo nuevo con el mismo nombre (`caliente.png`,
`altura.png`, `confinado.png`). Deben ser PNG cuadrados, con la figura en blanco sobre
fondo transparente.

### El logo

La aplicación espera el archivo `assets/logo-codelco-ssb.png`. **Si no existe, todo
funciona igual, simplemente sin logo**: ni la aplicación ni los PDF fallan.

Para incorporarlo, sube el archivo con ese nombre exacto a la carpeta `assets`. Conviene
que sea PNG con fondo transparente y unos 400 píxeles de ancho; la aplicación lo escala
sola. Si el fondo es blanco en vez de transparente, se verá un rectángulo blanco sobre
la barra oscura.

---

## 10. Cambiar la imagen del plano
...
│   ├── logo-codelco-ssb.png  ← deja aquí el logo con este nombre
│   └── iconos/             Iconos de los tres tipos de trabajo
├── css/
...
└── sql/
    ...
    ├── limpiar-datos-ejemplo.sql  Borra los registros de demostración
...
## 12. Preparado para lo que viene
