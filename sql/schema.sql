...
--
--    >>> BORRA ESTE BLOQUE ANTES DE USAR EL SISTEMA EN OPERACIÓN <<<
--    Sirve para comprobar que el mapa, los colores y el panel
--    funcionan. Usa un permiso_path ficticio, así que el botón
--    "Ver permiso" mostrará un aviso de archivo no disponible.
--
--    Para eliminarlos después:  delete from public.trabajos where registrado_por = 'Datos de ejemplo';
-- ------------------------------------------------------------
insert into public.trabajos
  (tipo, empresa, area, equipo, descripcion, n_trabajadores, supervisor,
   inicio, termino, x, y, permiso_path, permiso_tipo, registrado_por)
values
  ('caliente', 'Montajes Industriales Aconcagua', 'Fundición',
   'Horno de Limpieza de Escorias', 'Soldadura de refuerzo en estructura de soporte.',
   4, 'Rodrigo Salinas',
   (date_trunc('day', now() at time zone 'America/Santiago') + interval '8 hours') at time zone 'America/Santiago',
   (date_trunc('day', now() at time zone 'America/Santiago') + interval '19 hours 59 minutes') at time zone 'America/Santiago',
   0.372, 0.395, 'ejemplo/permiso-demo.pdf', 'application/pdf', 'Datos de ejemplo'),

  ('altura', 'Servicios Técnicos Puchuncaví', 'Refinería',
   'Nave de Electrorrefinación', 'Cambio de luminarias sobre pasarela a 9 metros.',
   3, 'Carla Meneses',
   (date_trunc('day', now() at time zone 'America/Santiago') + interval '8 hours 30 minutes') at time zone 'America/Santiago',
   (date_trunc('day', now() at time zone 'America/Santiago') + interval '17 hours') at time zone 'America/Santiago',
   0.283, 0.556, 'ejemplo/permiso-demo.jpg', 'image/jpeg', 'Datos de ejemplo'),

  ('confinado', 'Mantención Costa Norte', 'Planta de Ácido',
   'Estanque de almacenamiento N°2', 'Inspección interna y limpieza de fondo.',
   2, 'Iván Fuentes',
   (date_trunc('day', now() at time zone 'America/Santiago') + interval '9 hours') at time zone 'America/Santiago',
   (date_trunc('day', now() at time zone 'America/Santiago') + interval '15 hours 30 minutes') at time zone 'America/Santiago',
   0.041, 0.616, 'ejemplo/permiso-demo.jpg', 'image/jpeg', 'Datos de ejemplo');
