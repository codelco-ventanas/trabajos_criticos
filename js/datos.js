...
//  Reloj del servidor
//
//  No se puede confiar en la hora del equipo para decidir si un
//  permiso se está declarando con hora pasada: bastaría con atrasar
//  el reloj del celular. Se toma la hora del servidor de la cabecera
//  Date de una respuesta y se guarda la diferencia con el reloj
//  local. Si la consulta falla, se sigue con el reloj del equipo:
//  es preferible eso a impedir registrar un trabajo.
// ------------------------------------------------------------
let desfaseServidor = 0;

export async function sincronizaReloj() {
  try {
    ...
    if (cabecera) {
      const t = new Date(cabecera).getTime();
      if (!Number.isNaN(t)) desfaseServidor = t - Date.now();
    }
  } catch { /* se conserva el reloj local */ }
  return desfaseServidor;
}

/** Hora actual según el servidor, en milisegundos. */
export const ahoraServidor = () => Date.now() + desfaseServidor;
...
export async function cargaTrabajos(horasFinalizados = 12) {
  const margen = new Date(Date.now() - 60 * 60 * 1000).toISOString();
  // Se pide una ventana más amplia que la que se va a mostrar: un
  // permiso dura hasta 12 horas, así que un trabajo que terminó hace
  // 12 pudo empezar hace 24. El recorte fino se hace después, contra
  // la hora de término.
  const desde = new Date(Date.now() - (horasFinalizados + 14) * 60 * 60 * 1000).toISOString();
...
/** Marca o retira la prueba de comunicación radial. La hora la sella
 *  el servidor; aquí sólo se envía la decisión y quién la tomó. */
export async function marcaComunicacion(id, ok, actor = 'Administrador') {
  const { data, error } = await cliente
    .from('trabajos')
    .update({ comunicacion_ok: ok, comunicacion_por: ok ? actor : null,
              modificado_por: actor, modificado_en: new Date().toISOString() })
    .eq('id', id)
    .select()
    .single();
  if (error) throw traduce(error);
...
export async function guardaFotoLugar(id, archivo, actor, espacio = 1) {
  // Hay dos espacios para fotos del lugar: el 1 usa las columnas
  // lugar_path/lugar_tipo y el 2, lugar2_path/lugar2_tipo.
  const foto = await subeFotoLugar(archivo);
  const campos = espacio === 2
    ? { lugar2_path: foto.ruta, lugar2_tipo: foto.tipo }
    : { lugar_path: foto.ruta, lugar_tipo: foto.tipo };
  const { data, error } = await cliente
    .from('trabajos')
    .update({ ...campos,
...
export async function purgaPermisos() {
  ...
  const { data, error } = await cliente
    .from('trabajos')
    .select('id, permisos, lugar_path, lugar2_path')
  ...
  const rutas = data
    .flatMap(t => [...Object.values(t.permisos || {}).map(p => p?.path), t.lugar_path, t.lugar2_path])
  ...
  const { error: eMarca } = await cliente
    ...
    .update({ permisos: {}, permiso_path: null, permiso_tipo: null,
              ...
              lugar2_path: null, lugar2_tipo: null,
...
function traduce(error) {
  ...
  if (m.includes('observaciones_largo'))
    return new Error('Las observaciones no pueden superar los 600 caracteres.');
