...
export const subePermiso   = (archivo) => sube(archivo, 'el permiso de trabajo');
...
export async function purgaPermisos() {
  ...
  const { data, error } = await cliente
    .from('trabajos')
    .select('id, permiso_path, lugar_path')
    .or('permiso_path.not.is.null,lugar_path.not.is.null')
    .or(`termino.lt.${corte},and(cerrado.eq.true,cerrado_en.lt.${corte})`)
    .limit(200);

  if (error || !data?.length) return { archivos: 0 };

  // Se borran los dos archivos del trabajo: el permiso y la foto del
  // lugar, si existen.
  const rutas = data
    .flatMap(t => [t.permiso_path, t.lugar_path])
  ...
  const { error: eMarca } = await cliente
    .from('trabajos')
    .update({ permiso_path: null, permiso_tipo: null,
