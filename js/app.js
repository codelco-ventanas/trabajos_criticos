...
import {
  ...
  fechaHoraLarga, horaRelativa, soloHora, tiempoRestante, camposAhora, partesChile
} from './vigencia.js';
...
function pintaLista() {
  ...
  $('#tiradorTexto').textContent = activos.length
    ? `${activos.length} activos · ${personas} personas`
...
function tarjeta(t) {
  ...
  b.innerHTML = `
    <div class="tarjeta-fila">
      <span class="tarjeta-tipo">${tipo.glifo} ${tipo.corto}</span>
...
function anuncia(t) {
  ...
  avisa(`${TIPOS[t.tipo].glifo} Nuevo trabajo en ${t.area}: ${t.empresa}`, 'info');
}
...
function abreFicha(id) {
  ...
  $('#tituloFicha').innerHTML = `<span class="ficha-titulo">
      <span class="chip-tipo" style="--tono:${tipo.color}">${tipo.glifo} ${tipo.nombre}</span>
...
async function cargaSugerencias() {
  try {
    const s = await datos.sugerencias();
    llenaLista('#listaEmpresas', s.empresas);
    llenaLista('#listaAreas', s.areas);
    llenaLista('#listaSupervisores', s.supervisores);
  } catch { /* el autocompletado es opcional */ }
}
function llenaLista(sel, valores) {
  $(sel).innerHTML = valores.map(v => `<option value="${escapa(v)}"></option>`).join('');
}
...
function marcaUbicacion(punto) {
  ...
  if (punto) {
    ...
    txt.innerHTML = `Sin marcar
      <small>El punto indica dónde buscar en una emergencia.</small>`;
...
async function guarda() {
  ...
  try {
    ...
    if (estado.editando) {
      ...
      estado.trabajos.set(t.id, t);
      avisa(estadoDe(t) === 'PROGRAMADO'
        ? 'Trabajo programado. Aparecerá en el mapa a la hora de inicio.'
        : 'Trabajo registrado y visible en el mapa.');
    }
...
async function buscaHistorial() {
  ...
  cuerpo.innerHTML = '<tr><td colspan="7" style="color:var(--tenue);padding:20px">Buscando…</td></tr>';
  try {
    const filtros = {
    ...
    if (!historialActual.length) {
      cuerpo.innerHTML = '<tr><td colspan="7" style="color:var(--tenue);padding:20px">Sin resultados para esos filtros.</td></tr>';
      return;
    }
    cuerpo.innerHTML = '';
    historialActual.forEach(t => {
      ...
      const tr = document.createElement('tr');
      tr.innerHTML = `
        <td>${fechaHoraLarga(t.inicio)}</td>
        <td><span class="marca-tipo" style="--tono:${tipo.color}"></span>${tipo.corto}</td>
        <td>${escapa(t.empresa)}</td>
        <td>${escapa(t.area)} · ${escapa(t.equipo)}</td>
        <td>${escapa(t.supervisor)}</td>
        <td>${t.n_trabajadores}</td>
        <td><span class="chip-estado ${est.toLowerCase()}">${est}</span></td>`;
    ...
    cuerpo.innerHTML = `<tr><td colspan="7" style="color:#FFAA9C;padding:20px">${escapa(e.message)}</td></tr>`;
  }
}

function respaldoDelMapa() {
  const lista = paraElPanel();
  try {
    generaPDF(lista, {
      titulo: 'Respaldo de trabajos críticos',
      subtitulo: 'Incluye los trabajos en ejecución y los programados que aún no comienzan.',
      nombreArchivo: 'respaldo-trabajos-criticos'
    });
    avisa('Respaldo descargado.');
  } catch (e) { avisa(e.message, 'error'); }
}

function exportaHistorialPDF() {
  if (!historialActual.length) return avisa('Primero haz una búsqueda.', 'info');
  try {
    generaPDF(historialActual, {
      titulo: 'Historial de trabajos críticos',
      subtitulo: `${historialActual.length} registros según los filtros aplicados.`,
      nombreArchivo: 'historial-trabajos-criticos',
      conResumen: false
    });
...
function conectaEventos() {
  ...
  $('#btnRespaldo').addEventListener('click', respaldoDelMapa);
  ...
  $('#tirador').addEventListener('click', () => {
    const p = $('#panel');
    const abiertoAhora = p.classList.toggle('desplegado');
    $('#tirador').setAttribute('aria-expanded', String(abiertoAhora));
  });
}
