...
/** Momento en que la actividad dejó de estar vigente: su hora de
 *  término, o el cierre anticipado si un administrador la finalizó
 *  antes. */
const momentoFin = (t) =>
  new Date(t.cerrado && t.cerrado_en ? t.cerrado_en : t.termino).getTime();
...
function abreFicha(id) {
  ...
  $('#cuerpoFicha').innerHTML = `
    ...
    <div class="interruptor-radio solo-admin" id="cajaRadio">
      <span class="texto-radio">¿Se realizó la prueba de comunicación?
        <small>Al marcar Sí queda registrada la hora del servidor y su responsable.</small>
      </span>
  ...
  $$('#cajaRadio .conmutador button').forEach(b => b.addEventListener('click', () =>
    cambiaComunicacion(t.id, b.dataset.valor === 'si')));
...
async function cambiaComunicacion(id, ok) {
  ...
  $$('#cajaRadio .conmutador button').forEach(b => { b.disabled = true; });
  try {
    const t = await datos.marcaComunicacion(id, ok, 'Administrador');
    estado.trabajos.set(t.id, t);
    dibujaTodo();
    abreFicha(t.id);
    avisa(ok ? 'Prueba de comunicación registrada.' : 'Prueba de comunicación retirada.', 'info');
  } catch (e) {
    avisa(e.message, 'error');
    $$('#cajaRadio .conmutador button').forEach(b => { b.disabled = false; });
  }
}
