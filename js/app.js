...
const estado = {
  ...
  archivoLugar: null,      // primera foto del lugar en el formulario
  archivoLugar2: null,     // segunda foto del lugar en el formulario
...
function pintaSala(activos) {
  ...
  lista.forEach(t => {
    ...
    tr.innerHTML = `
      ...
      <td>${escapa(t.supervisor)}${t.supervisor_telefono
        ? `<br><a class="telefono" href="tel:${escapa(telefonoMarcable(t.supervisor_telefono))}"
              onclick="event.stopPropagation()">${escapa(t.supervisor_telefono)}</a>` : ''}</td>
...
function abreFicha(id) {
  ...
  $('#cuerpoFicha').innerHTML = `
    <div class="datos">
      ...
      ${t.observaciones ? `
      <div class="dato ancho observaciones">
        <div class="dato-clave">Observaciones para emergencia</div>
        <div class="dato-valor">${escapa(t.observaciones)}</div>
      </div>` : ''}
      <div class="dato">
      ...
      <div class="dato">
        ...
        ${t.supervisor_telefono
          ? `<a class="telefono" href="tel:${escapa(telefonoMarcable(t.supervisor_telefono))}">
               ${escapa(t.supervisor_telefono)}</a>`
          : ''}
      </div>
      <div class="dato">
    ...
    <div class="bloque-permiso">
      ${fotosLugarDe(t).map((f, i, todas) =>
        `<button class="btn ver-lugar" data-espacio="${f.espacio}"
                 style="width:100%;justify-content:center;margin-bottom:8px">
           Ver foto del lugar${todas.length > 1 ? ' ' + (i + 1) : ''}
         </button>`).join('')}
      ${(est === 'FINALIZADO' || !espacioLibre(t)) ? '' :
        `<button class="btn" id="btnAgregarLugar" style="width:100%;justify-content:center;margin-bottom:8px">
           ${fotosLugarDe(t).length ? 'Agregar segunda foto del lugar' : 'Agregar foto del lugar'}
         </button>
         ${fotosLugarDe(t).length ? '' : `<p class="sugerencia-lugar">Se recomienda incorporarla
           siempre: agiliza la respuesta ante una emergencia.</p>`}`}
  ...
  $$('#cuerpoFicha .ver-lugar').forEach(b => b.addEventListener('click', () => {
    const f = fotosLugarDe(t).find(x => String(x.espacio) === b.dataset.espacio);
    if (f) verArchivo(f.path, f.mime, 'Fotografía del lugar');
  }));
...
function describeCambios(f) {
  ...
  const campos = Object.keys(c)
    .filter(k => !['comunicacion_en', 'comunicacion_por', 'lugar_tipo', 'lugar2_tipo',
                    'permiso_tipo'].includes(k))
    .map(nombreCampo);
  return campos.length ? ' · ' + campos.join(', ') : '';
}

/** Fotos del lugar disponibles, como [{path, mime, espacio}]. */
function fotosLugarDe(t) {
  return [
    { path: t.lugar_path,  mime: t.lugar_tipo,  espacio: 1 },
    { path: t.lugar2_path, mime: t.lugar2_tipo, espacio: 2 }
  ].filter(f => f.path);
}

/** Primer espacio libre para una foto del lugar, o null si ya hay dos. */
const espacioLibre = (t) => !t.lugar_path ? 1 : (!t.lugar2_path ? 2 : null);

/** Deja sólo dígitos y el + inicial, para el enlace de llamada. */
const telefonoMarcable = (tel) => {
...
const telefonoValido = (tel) => {
  ...
  return d.length >= 8 && d.length <= 15 && /^[\d\s+()\-.]+$/.test(String(tel).trim());
};
...
const nombresCampos = {
  ...
  permisos: 'permisos', lugar_path: 'foto del lugar', lugar2_path: 'segunda foto del lugar',
  supervisor_telefono: 'teléfono del supervisor', observaciones: 'observaciones',
...
async function cambiaComunicacion(id, ok) {
  ...
  // Una actividad terminada ya no admite cambios en su prueba de
  // comunicación: el dato quedó fijado al momento de ejecutarse.
  if (estadoDe(actual) === 'FINALIZADO') {
    return avisa('El trabajo ya finalizó: su prueba de comunicación no se puede modificar.', 'error');
  }
...
function abreFotoLugar(id) {
  trabajoParaFoto = id;
  const t = estado.trabajos.get(id);
  espacioParaFoto = (t && espacioLibre(t)) || 1;
...
async function guardaFotoLugar() {
  ...
  try {
    const t = await datos.guardaFotoLugar(
      trabajoParaFoto, estado.archivoLugarFicha, $('#quienAgrega').value.trim(), espacioParaFoto);
    estado.trabajos.set(t.id, t);
    dibujaTodo();
    cierra('veloLugar');
...
function abreFormularioEdicion(t) {
  ...
  $('#telefono').value = t.supervisor_telefono || '';
  ...
  estado.lugarPrevio = { 1: !!t.lugar_path, 2: !!t.lugar2_path };
...
const NOTA_LUGAR_PREVIA = 'Ya hay una foto cargada. Adjunte otra sólo si desea reemplazarla.';

function marcaAdjuntoLugar(archivo) {
  marcaCaja('#cajaLugar', '#textoLugar', '#btnAdjuntarLugar', archivo,
            estado.lugarPrevio?.[1] ? NOTA_LUGAR_PREVIA : NOTA_LUGAR);
}

function marcaAdjuntoLugar2(archivo) {
  marcaCaja('#cajaLugar2', '#textoLugar2', '#btnAdjuntarLugar2', archivo,
            estado.lugarPrevio?.[2] ? NOTA_LUGAR_PREVIA
              : 'Opcional. Útil para mostrar el acceso o una vista más amplia del lugar.');
}
...
async function guarda() {
  ...
  if (!$('#telefono').value.trim())
    return muestraError('Indica el teléfono de contacto del supervisor.');
  if (!telefonoValido($('#telefono').value))
    return muestraError('El teléfono del supervisor no parece válido. Use sólo números, por ejemplo +56 9 1234 5678.');
  ...
  try {
    ...
    let lugar2 = null;
    if (estado.archivoLugar2) {
      boton.textContent = 'Subiendo segunda foto del lugar…';
      lugar2 = await datos.subeFotoLugar(estado.archivoLugar2);
    }
    ...
    const comun = {
      ...
      supervisor_telefono: $('#telefono').value.trim(),
    ...
    if (lugar2) { comun.lugar2_path = lugar2.ruta; comun.lugar2_tipo = lugar2.tipo; }
...
function exportaCSV() {
  ...
  const cab = ['Tipo', 'Empresa', 'Area', 'Equipo', 'Descripcion', 'Trabajadores',
               'Supervisor', 'Telefono supervisor', 'Observaciones',
               'Inicio', 'Termino real', 'Estado', 'Registrado por', 'Creado en'];
  const filas = historialActual.map(t => [
    nombreTipos(t, true), t.empresa, t.area, t.equipo, t.descripcion, t.n_trabajadores,
    t.supervisor, t.supervisor_telefono || '', t.observaciones || '',
    fechaHoraLarga(t.inicio), fechaHoraLarga(terminoReal(t)),
...
function conectaEventos() {
  ...
  $('#btnAdjuntarLugar2').addEventListener('click', () => $('#archivoLugar2').click());
  $('#archivoLugar2').addEventListener('change', (e) => {
    const f = e.target.files[0];
    const problema = datos.revisaArchivo(f);
    if (problema) { avisa(problema, 'error'); e.target.value = ''; return; }
    estado.archivoLugar2 = f;
    marcaAdjuntoLugar2(f);
  });
  ...
  $('#btnEliminar').addEventListener('click', async () => {
    ...
    try {
      await datos.eliminaTrabajo(t.id, ...permisosDe(t).map(([, p]) => p.path),
                                 t.lugar_path, t.lugar2_path);
