...
// Margen de tolerancia hacia atrás al registrar un trabajo nuevo.
// Sirve para que no se rechace un formulario que se demoró unos
// minutos en completarse; no alcanza para declarar en forma
// retroactiva una actividad que ya estaba en ejecución.
export const MARGEN_ATRASO_MIN = 5;

/**
 * Comprueba que un trabajo nuevo no se esté declarando con hora ya
 * pasada. Sólo se aplica al registrar: al editar, lo normal es que la
 * actividad ya haya comenzado.
 */
export function validaNoPasado(inicio, ahora) {
  const limite = ahora - MARGEN_ATRASO_MIN * 60 * 1000;
  if (inicio.getTime() < limite) {
    const p = partesChile(new Date(ahora));
    return {
      ok: false,
      mensaje: `La hora de inicio ya pasó. Son las ` +
        `${dosDigitos(p.hora)}:${dosDigitos(p.minuto)} según el servidor, y un trabajo ` +
        `crítico debe declararse antes de comenzar, no después.`
    };
  }
  return { ok: true };
}
...
export function camposAhora(referencia = Date.now()) {
  const d = new Date(Math.ceil(referencia / 300000) * 300000);
