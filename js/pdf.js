...
/** Hora de término real: la del cierre si un administrador finalizó
 *  la actividad antes de su plazo estimado. */
...
export async function pdfDeclaracion(t) {
  ...
  doc.autoTable({
    ...
    body: [
      ...
      ['Observaciones para emergencia', t.observaciones || 'Sin observaciones'],
      ['Número de trabajadores', String(t.n_trabajadores)],
      ['Supervisor responsable', t.supervisor],
      ['Teléfono del supervisor', t.supervisor_telefono || 'No informado'],
      ['Fecha del trabajo', soloFecha(t.inicio)],
      ['Hora de inicio', soloHora(t.inicio)],
      ['Hora de término estimada', soloHora(t.termino)],
      ...(t.cerrado && t.cerrado_en
        ? [['Hora de término real', `${soloFecha(t.cerrado_en)} ${soloHora(t.cerrado_en)}`]]
        : []),
      ['Prueba de comunicación', t.comunicacion_ok
        ? `Realizada el ${soloFecha(t.comunicacion_en)} a las ${soloHora(t.comunicacion_en)}`
        : 'Sin registrar al momento de emisión de este registro'],
      ...
      ['Fotografías del lugar', (() => {
        const n = [t.lugar_path, t.lugar2_path].filter(Boolean).length;
        return n ? `${n} adjunta${n > 1 ? 's' : ''}` : 'Sin fotografías al momento de emisión';
      })()],
  ...
  doc.text(`Respaldo emitido el ${ahora.largo}.`, 14, y + 11);
...
export async function pdfHistorial(trabajos, filtros = []) {
  ...
  doc.autoTable({
    ...
    head: [['Fecha', 'Inicio', 'Término', 'Tipo', 'Empresa',
            'Área / equipo', 'Actividad', 'Pers.', 'Radio', 'Hora radio', 'Supervisor']],
    body: trabajos.map(t => ([
      soloFecha(t.inicio),
      soloHora(t.inicio),
      soloHora(terminoReal(t)),
      tiposDe(t).map(x => TIPOS[x]?.corto || x).join('\n'),
      t.empresa || '',
      `${t.area || ''}\n${t.equipo || ''}`,
      t.descripcion || '',
      String(t.n_trabajadores ?? ''),
      t.comunicacion_ok ? 'Sí' : 'No',
      t.comunicacion_ok ? soloHora(t.comunicacion_en) : '—',
      [t.supervisor || '', t.supervisor_telefono || ''].filter(Boolean).join('\n')
    ])),
    theme: 'grid',
    styles: {
    ...
    // La columna de tipo se dibuja a mano, línea por línea, para que
    // en una actividad combinada cada tipo lleve su propio color:
    // autoTable sólo admite un color por celda.
    didParseCell: (d) => {
    ...
    didDrawCell: (d) => {
      ...
      const alto = 3.6;
      const y0 = d.cell.y + (d.cell.height - tipos.length * alto) / 2 + 2.6;
      doc.setFont(FUENTE, 'bold');
      doc.setFontSize(8);
      tipos.forEach((x, i) => {
        doc.setTextColor(...(RGB[x] || RGB.texto));
        doc.text(TIPOS[x]?.corto || x, d.cell.x + d.cell.padding('left'), y0 + i * alto);
      });
      doc.setTextColor(...RGB.texto);
    },
    margin: { left: 14, right: 14, top: 40 }
