...
export function tiposDe(t) {
  const lista = Array.isArray(t.tipos) && t.tipos.length ? t.tipos : (t.tipo ? [t.tipo] : []);
  return ORDEN.filter(x => lista.includes(x));
}

/** Firma visual del marcador: si no cambia, no hace falta reconstruirlo. */
const firmaDe = (t) => `${tiposDe(t).join('+')}|${estadoDe(t)}`;

/** Fondo del marcador: un color si es un solo tipo, o bandas
 *  verticales si son varios. Se usan bandas y no sectores porque los
 *  iconos van en fila: así cada icono queda sobre su propio color y se
 *  ve de inmediato a qué tipo corresponde. */
function fondoDe(tipos) {
  if (tipos.length === 1) return TIPOS[tipos[0]].color;
  const paso = 100 / tipos.length;
  const bandas = tipos.map((t, i) =>
    `${TIPOS[t].color} ${i * paso}% ${(i + 1) * paso}%`).join(', ');
  return `linear-gradient(90deg, ${bandas})`;
}

function iconoDeTrabajo(trabajo) {
  const tipos = tiposDe(trabajo);
  ...
  const iconos = tipos.map(t => `<i class="icono-tipo ${t}"></i>`).join('');
  const lado = tipos.length > 1 ? 40 : 33;
  return L.divIcon({
    className: 'marcador-envoltorio',
    html: `<div class="marcador ${estado.toLowerCase()} tipos-${tipos.length}"
                style="--tono:${TIPOS[tipos[0]].color}; --fondo:${fondoDe(tipos)}; --fase:${fase}s">
             <span class="nucleo">${iconos}</span>
           </div>`,
    iconSize: [lado, lado],
...
function iconoDeGrupo(grupoMarcadores) {
  ...
  // Una actividad con dos tipos aporta a los dos sectores del anillo,
  // aunque en el centro se cuente como un solo trabajo.
  hijos.forEach(m => (m.options.tiposTrabajo || []).forEach(t => cuenta[t]++));
  const total = Object.values(cuenta).reduce((a, b) => a + b, 0) || hijos.length;
...
export function pintaTrabajos(trabajos) {
  ...
  trabajos.forEach((t) => {
    ...
    if (existente) {
      ...
      existente.options.tiposTrabajo = tiposDe(t);
      existente.trabajo = t;
    } else {
      const m = L.marker(aLatLng(t.x, t.y), {
        icon: iconoDeTrabajo(t),
        tiposTrabajo: tiposDe(t),
        riseOnHover: true
      });
      m.trabajo = t;
      m._firma = firmaDe(t);
